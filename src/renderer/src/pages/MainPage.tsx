...
const MainPage: React.FC<Props> = ({ setAppState, setNik }) => {
  ...
  const showSuggestions = suggestions.length > 0 && !selectedNik;

  /* ---------- render ---------- */
  return (
    <div className="flex flex-col items-center justify-center w-screen h-screen bg-gray-100 px-6 space-y-4">
        <div className="flex flex-col items-center text-center max-w-2xl space-y-1">
      ...
      {error && (
        <p className="text-sm text-red-600 font-medium bg-red-50 border border-red-200 rounded-md p-2 max-w-lg text-center">
          {error}
        </p>
      )}
    </div>
